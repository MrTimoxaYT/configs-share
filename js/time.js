...
function time_is_widget() {
    // Найдите и замените "Россия" на "Москва" в коде шаблона
    a["template"] = ct.replace("SUN", tl + sF + "</span>").replace("TIME", tl + tF + "</span>").replace("DATE", '<span onclick="window.location=\'https://time.is/calendar\'" title="https://time.is/calendar">' + dF + '</span>').replace("Россия", "Москва");
